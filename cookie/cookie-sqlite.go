...
func NewSqliteJar(dbname string) (*SqliteJar) {
    ...
    if dbname == "" {
        ...
        utils.Warn("Sqlite3 filename not be set! Set default : %s", dbname)
    }
    ...
    if err == nil {
        ...
        if err != nil {
            utils.Fatal("Open sqlite3 database failed! %s", err)
        }
        re, err := conn.Query(`SELECT value from meta WHERE key="s"`)
        defer re.Close()

        if err != nil {
            utils.Error("Faild load database s! %s", err)
        }
        var s string
        rs := re.Next()
        if !rs {
            utils.Error("Faild load database s!")
        }
        err = re.Scan(&s)
        if err != nil {
            utils.Error("Faild load database s!")
        ...
        if err != nil {
            utils.Fatal("Create table error! %s", err.Error())
        }
        s := salt(32)
        _, err = conn.Exec("INSERT INTO meta(key,value) VALUES('s',?)", s)

        if err != nil {
            utils.Fatal("Failed write to database! %s", err.Error())
        }
        ...
        utils.Fatal("Unresolved fatal %s", err)
    }
    return nil
}
...
func (j *SqliteJar) saveCookies(u *url.URL, cookies []*http.Cookie) {
    ...
    utils.Debug("Save cookies to database")
    delete_query := `
      DELETE FROM cookies
    ...
    if err != nil {
        utils.Warn("Create delete prepare err : %s", err)
    }
    insert_smt, err := j.conn.Prepare(insert_query)
    if err != nil {
        utils.Warn("Create insert prepare err : %s", err)
    }
    ...
    for _, cookie := range cookies {
        ...
        if err != nil {
            utils.Warn("Exec sql %s error : %s", delete_query, err)
        }
        ...
        if err != nil {
            utils.Warn("Exec sql %s error : %s", insert_query, err)
        }
    }
...
func (j *SqliteJar) loadCookies(u *url.URL) ([]*http.Cookie) {
    ...
    defer result.Close()
    if err != nil {
        utils.Warn("Load cookies from database failed : %s", err)
    }
